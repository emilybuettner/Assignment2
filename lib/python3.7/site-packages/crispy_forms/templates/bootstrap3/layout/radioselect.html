...
<div class="controls {{ field_class }}" {% if flat_attrs %} {{ flat_attrs|safe }}{% endif %}>
    ...
    <div class="radio">{% endif %}
        <label for="id_{{ field.id_for_label }}_{{ forloop.counter }}"
            ...
            <input type="radio" {% if choice.0|stringformat:"s" == field.value|default_if_none:""|stringformat:"s" %}
            checked="checked"{% endif %} name="{{ field.html_name }}" id="id_{{ field.id_for_label }}_{{ forloop.counter
